...
import { TrafficTrend, FunnelBars } from "@/components/charts";
import { fmtNum, fmtPct } from "@/lib/format";

export default function TrafficPage() {
  const { metrics, agg, rangeLabel, loading } = useDash();

  const hasTraffic = agg.sessions > 0 || agg.visitors > 0;
  const orders = agg.orders_count;
  // Consistent with the Overview: conversion is orders / unique NS Home visitors.
